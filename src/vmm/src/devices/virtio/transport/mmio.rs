...
pub(crate) mod tests {
    ...
    fn test_new() {
        ...
        assert_eq!(d.locked_device().queue_config(d.queue_select as usize).unwrap().size, 16);
    ...
    fn test_bus_device_write() {
        ...
        assert_eq!(d.locked_device().queue_config(0).unwrap().desc_table_address.0, 0);
        write_le_u32(&mut buf[..], 123);
        d.write(0x0, 0x80, &buf[..]);
        assert_eq!(d.locked_device().queue_config(0).unwrap().desc_table_address.0, 123);
        d.write(0x0, 0x84, &buf[..]);
        assert_eq!(
            d.locked_device().queue_config(0).unwrap().desc_table_address.0,
            123 + (123 << 32)
        );

        assert_eq!(d.locked_device().queue_config(0).unwrap().avail_ring_address.0, 0);
        write_le_u32(&mut buf[..], 124);
        d.write(0x0, 0x90, &buf[..]);
        assert_eq!(d.locked_device().queue_config(0).unwrap().avail_ring_address.0, 124);
        d.write(0x0, 0x94, &buf[..]);
        assert_eq!(
            d.locked_device().queue_config(0).unwrap().avail_ring_address.0,
            124 + (124 << 32)
        );

        assert_eq!(d.locked_device().queue_config(0).unwrap().used_ring_address.0, 0);
        write_le_u32(&mut buf[..], 125);
        d.write(0x0, 0xa0, &buf[..]);
        assert_eq!(d.locked_device().queue_config(0).unwrap().used_ring_address.0, 125);
        d.write(0x0, 0xa4, &buf[..]);
        assert_eq!(
            d.locked_device().queue_config(0).unwrap().used_ring_address.0,
            125 + (125 << 32)
        );
    ...
    fn test_queue_config_immutable_after_activation() {
        ...
        let desc_before = dev.locked_device().queue_config(0).unwrap().desc_table_address;
        let avail_before = dev.locked_device().queue_config(0).unwrap().avail_ring_address;
        let used_before = dev.locked_device().queue_config(0).unwrap().used_ring_address;
        ...
        assert_eq!(dev.locked_device().queue_config(0).unwrap().size, size_before);

        // QueueReady (0x44)
        write_le_u32(&mut buf, 0);
        dev.write(0x0, 0x44, &buf);
        assert_eq!(dev.locked_device().queue_config(0).unwrap().ready, ready_before);
        ...
        assert_eq!(
            dev.locked_device().queue_config(0).unwrap().desc_table_address,
            desc_before
        );
        ...
        assert_eq!(
            dev.locked_device().queue_config(0).unwrap().avail_ring_address,
            avail_before
        );
        ...
        assert_eq!(
            dev.locked_device().queue_config(0).unwrap().used_ring_address,
            used_before
        );
    }
