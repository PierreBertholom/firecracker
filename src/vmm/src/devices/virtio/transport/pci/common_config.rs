...
impl VirtioPciCommonConfig {
    ...
    fn write_common_config_dword(
        ...
        match offset {
            ...
            QUEUE_USED_LO => self.update_queue_field(&mut *locked_device, |q| {
                lo(&mut q.used_ring_address, value)
            }),
            QUEUE_USED_HI => self.update_queue_field(&mut *locked_device, |q| {
    ...
    fn with_queue_mut<F: FnOnce(&mut QueueConfig)>(
        &self,
        device: &mut dyn VirtioDevice,
        f: F,
    ) {
