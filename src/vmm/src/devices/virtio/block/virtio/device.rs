...
use super::{BLOCK_QUEUE_SIZE, SECTOR_SHIFT, SECTOR_SIZE, VirtioBlockError};
...
impl VirtioDevice for VirtioBlock {
    ...
    fn _reset(&mut self) -> bool {
        ...
        let (resources, worker_handles) = match state {
            BlockState::Active(ActiveBlock::Threaded(active)) => {
                assert_eq!(
            ...
            BlockState::Configuring(mut resources, worker_handles) => {
                for resource in &mut resources {
                    ...
                    resource.is_io_engine_throttled = false;
                }
...
mod tests {
    ...
    use crate::devices::virtio::block::virtio::test_utils::{
        default_block, default_config, default_mq_block, default_threaded_block,
    ...
    fn test_threaded_reset_worker_failure() {
        let mut block = default_block(FileEngineType::Sync);
    ...
    fn test_mq_update_disk() {
        for engine in [FileEngineType::Sync, FileEngineType::Async] {
            ...
            // Reset hands the resources back so every worker's disk can be checked.
            assert!(block.reset());
            for resources in block.resources() {
