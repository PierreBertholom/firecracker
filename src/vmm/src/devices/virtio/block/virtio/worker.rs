...
impl BlockWorker {
    ...
    pub(crate) fn prepare_save(&mut self) {
        // A failed fsync is a storage error and the snapshot still goes ahead, as legacy inline
        // does. Any other error is a broken ring or a logic bug, so there is no recovery.
...
impl ThreadedWorker {
    ...
    fn reset_worker(&mut self, ops: &mut EventOps) {
        let response = match std::mem::replace(&mut self.state, WorkerState::Parked) {
            ...
            state => {
                warn!("Reset requested while block worker is not active");
                self.state = state;
    ...
    fn pause_worker(&mut self, ops: &mut EventOps) {
        let response = match std::mem::replace(&mut self.state, WorkerState::Parked) {
            ...
            state => {
                warn!("Pause requested while block worker is not active");
                self.state = state;
                ControlResponse::InvalidState(
    ...
    fn send_queue_state(&self) {
        let response = match &self.state {
            ...
            _ => {
                warn!("Queue state requested while block worker is not paused");
                ControlResponse::InvalidState(
...
mod tests {
    ...
    fn test_control_msg_batch() {
        ...
        let worker = BlockWorker {
            ...
            is_blocked: block.rate_limiter().clone_blocked_flag(),
    ...
    fn test_kick_while_paused() {
        ...
        handle.start(worker);
        handle.pause();
        let paused_queue_state = handle.get_queue_state();
        ...
        handle.kick(false);
        assert_eq!(handle.get_queue_state(), paused_queue_state);
