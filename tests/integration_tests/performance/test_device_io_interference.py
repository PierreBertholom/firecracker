...
def submit_fio(
        microvm, executor, guest_dev, mode, results_dir, runtime, warmup
):
...
@pytest.mark.parametrize("pool_size", [0, 1, 2], ids=["pool0", "pool1", "pool2"])
...
def test_block_net_throughput_interference(
    ...
    * ``concurrent``  -> both at once, on overlapping steady-state windows
    ...
    Swept across ``pool_size``: 0 = legacy (devices on the shared VMM thread),
    1 = all devices on one dedicated worker thread, 2 = one worker per device.
    """
    vm = uvm
    vm.spawn(log_level="Info", emit_metrics=True)
    vm.basic_config(vcpu_count=vcpus, mem_size_mib=GUEST_MEM_MIB, pool_size=pool_size)
    vm.add_net_iface()
...
def test_network_latency_under_block_load(
    ...
    Swept across ``pool_size`` (0 = legacy shared thread, 1/2 = worker pool) to
    see whether moving block I/O off the VMM thread reduces the net latency tail.
    """
    vm = uvm
    vm.spawn(log_level="Info", emit_metrics=True)
    vm.basic_config(vcpu_count=vcpus, mem_size_mib=GUEST_MEM_MIB, pool_size=pool_size)
...
def test_multi_block_throughput(
    ...
    Swept across ``pool_size``: with ``dual`` + ``pool2`` each scratch device
    lands on its own worker thread, which is the case the pool is meant to win.
    """
    vm = uvm
    vm.spawn(log_level="Info", emit_metrics=True)
    vm.basic_config(vcpu_count=vcpus, mem_size_mib=GUEST_MEM_MIB, pool_size=pool_size)
    ...
    vm.add_drive("scratch_b", fs_b.path, io_engine=io_engine)
    fs_c = drive_tools.FilesystemFile(
        os.path.join(vm.fsfiles, "scratch_c"), BLOCK_DEVICE_SIZE_MB
    )
    vm.add_drive("scratch_c", fs_c.path, io_engine=io_engine)
    vm.start()
